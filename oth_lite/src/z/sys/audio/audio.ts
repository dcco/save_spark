...
export const GAudio: AudioIface = {
	curTrack: null,
	curHowl: null,
	nextTrack: null,
	volume: 0,
	...
	incVol: function(n) {
		...
		if (this.extVol < 0) { this.extVol = 0; }
		else if (this.extVol > 100) { this.extVol = 100; }
	},
	update: function() {
		...
		if (this.curTrack !== this.nextTrack) {
			...
			if (this.volume === 0) {
				...
				if (this.nextTrack === null) { 
					...
					if (this.curHowl !== null) { this.curHowl.stop(); }
					this.curHowl = null;
				// otherwise, begin playback
				} else {
					// if track is not loaded, wait
					var track = GAudioList.get(this.nextTrack);
					if (track === undefined) return;
					...
					if (this.curTrack === null) { this.curHowl.volume(track.vol * vp); }
					else { this.curHowl.volume(0); }
					this.curTrack = this.nextTrack;
				}
			}
			// otherwise, lower volume
			if (this.curHowl === null) return;
			if (this.volume > 0) this.volume = this.volume - 0.1;
			if (this.volume < 0) this.volume = 0;
		// otherwise, begin fading in new track
		} else if (this.curTrack !== null) {
			if (this.curHowl === null || this.volume === this.tarVol) return;
			if (!this.curHowl.playing()) { this.curHowl.play(); }
			if (this.volume < this.tarVol) this.volume = this.volume + (0.1 * this.tarVol);
			if (this.volume > this.tarVol) this.volume = this.tarVol;
		}
	}
}